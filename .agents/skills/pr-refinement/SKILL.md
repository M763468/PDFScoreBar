...
1) Run `bash .agents/skills/pr-refinement/run.sh <pr_number>` to fetch PR comments and status into an artifact.
...
6) Verify the fixes with `make format`, `make lint`, `make test-fast`, and any PR-specific pytest, smoke, or validation-policy commands required by the change.
...
- `bash .agents/skills/pr-refinement/run.sh`: script to fetch PR refinement data into `artifacts/`
...
## Example commands
- `bash .agents/skills/pr-refinement/run.sh 123`
- `gh pr checkout 123`
