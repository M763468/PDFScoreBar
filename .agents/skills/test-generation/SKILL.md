...
6) Run `./run.sh [test_path]` (or `make test`) to verify tests and output results to `artifacts/test_results.txt`. For real-data parity check, run the specific verification command.
...
## Example commands
- `./run.sh tests/test_pipeline_detection.py`
- `make test`
