...
- **Artifacts First**: Standard outputs for repetitive or verbose commands MUST be redirected to `artifacts/` to prevent polluting the context window (e.g., `pytest > artifacts/test_results.txt`). Pipeline, evaluation, experiment, and generated model/data outputs MUST go under `logs/`.
...
- **Trusted Commands**: The following commands are pre-approved and do not require explicit user confirmation for each execution:
  ...
  - `bash .agents/skills/<skill>/run.sh ...`: All `run.sh` scripts within `.agents/skills/` subdirectories. There is no repository-root `./run.sh`; run skill scripts from the repository root with their full `.agents/skills/<skill>/run.sh` path.
