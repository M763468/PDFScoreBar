...
2. `manual_review/residual_manual_review_template.csv` はユーザー入力分を保持したうえで Codex が暫定補完済み。集計では `fp_divisi_spanning=62`, `fp_out_of_staff=29`, `fp_near_gt_duplicate=25`, `fp_internal_false_bar=9`, `fn_right_edge_missing=5`, `fn_cnn_low_score=5`, `fn_candidate_stage_miss=3`。
3. 次セッションは `docs/ISSUE120_NEXT_SESSION_HANDOFF_PROMPTS.md` のプロンプトに従って分割する。順序は staff-region filter 調査、divisi/system-spanning FP 混入経路追跡、FN 残課題 trace。
