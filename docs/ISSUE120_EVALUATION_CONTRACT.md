...
ISSUE120_STAFF_UNITS_JSON=logs/issue313/staff_units.json
...
PYTHONPATH=. python3 tools/issue120/eval_full68_from_intermediates.py \
  ...
  --score-threshold 0.1 \
  --staff-units-json logs/issue313/staff_units.json \
...
Before a canonical center-anchor evaluation, materialize the ignored, page-level
manifest from the audited staff-mask inputs:

```bash
PYTHONPATH=. python3 tools/issue313/build_staff_units_manifest.py \
  --audit-json logs/issue313_audit/audit.json \
  --output logs/issue313/staff_units.json
```
...
scales. `--legacy-fixed-12px` exists only in the direct evaluator for historical
reproduction; it is not the canonical contract.
