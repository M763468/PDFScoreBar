...
## 0. 全体的な Residuals (Raw FP/FN) の推移

最終的な `filtered_cnn_json` レイヤーにおける Raw FP / FN の総件数は以下の通り劇的な変化を示しました。

| 指標 | 対策前 (vov: 0.0) | 対策後 (vov: 0.5) | 増減 |
| :--- | :---: | :---: | :---: |
| **FP 総数** | 125 | 14 | **-111** |
| **FN 総数** | 20 | 28 | **+8** |

*   当初の目的であった `tall_or_system_spanning_fp` は 58件から 0件へと 100% の一掃に成功しました。
*   `remote_fp` カテゴリも 42件から 1件 へと劇的に減少しました。
*   しかしながら、強力なフィルタリングの代償として、**新たに 8 件の FN（検知漏れ）が発生** してしまいました。小節線のカウントにおいて、FN（検知漏れ）は後続の小節番号をすべてずらしてしまう**極めて致命的なエラー**です。

## 1. フィルタ適用によって新たに発生した FN (New FNs)

対策前（閾値 0.0）では検出できていたものの、対策後（閾値 0.5）で新たに FN となってしまった致命的な 8 件を分析します。
...
*考察:* 
このケースは、たまたまインクのノイズ（あるいは文字など）が五線領域の Y 座標と被る位置（`vov >= 0.5`）に存在し、かつ形状が縦線に似ていたため CNN スコアも 0.835 と高く出たものです。段を跨ぐ線ではなく「五線上に乗っている縦長のノイズ」であるため、今回の `local_staff_overlap` フィルタでは原理的に防げません。

## 3. 結論 (Prompt 2 の総括と反省)

- **FPの一掃:** `staff_vov_threshold: 0.5` のフィルタを適用した結果、ターゲットであった「段を跨ぐ / ギャップに浮かぶ巨大な FP（System-spanning FP）」を完璧に除去し、**全体の FP を 125件から 14件へと激減させる** ことに成功しました。
- **致命的な副作用の認識:** 一方で、このフィルタ強化の副作用として **新たに 8件の FN（検知漏れ）** を引き起こしてしまいました。小節線カウントというタスクの性質上、1件のFNの発生は小節番号全体を崩壊させる致命的な違反であり、「割合が小さいから許容できる」という問題ではありません。
- **次のステップ:** 
  これをもって Prompt 2 の作業（Divisi FPの除去）は完了とみなしますが、**今回の操作で発生させてしまった 8 件の FN については、必ず救済できなければなりません**。
  次の Prompt 3（FN側の残課題調査）において、これらの新規 FN をどのように安全に Rescue するか（CNNのスコア閾値や、VOVによる足切り後の Rescue 機構など）を厳密に審査し、対策を講じます。
