...
`src/pipeline/detection/restored_orchestrator.py`. The generic
...
PDF or persisted page images
  ...
  -> dense detector route dispatch
       -> page-local verified source worker
            ...
            -> current x4 support owner
                 -> Real-ESRGAN x4, persisted
                 -> current-runtime HOMR on persisted x4 image
                 -> OMR-DLN on the persisted x4 image
            -> hybrid consensus
...
- **Owner:** `src/pipeline/detection/current_support_worker.py`.
- **Input:** a persisted Real-ESRGAN x4 image generated by `current_sr_worker`.
...
The current x4 support worker deliberately separates its heavy memory phases:
Real-ESRGAN -> current HOMR -> OMR-DLN. The worker records those phase boundaries and does
not keep the HOMR state resident during the SR peak.

## Detector consensus and dense candidate route

`VerifiedProfileHybridDetector` combines:
...
The old pinned HOMR-on-x4 inference is not part of this consensus. The current x4 HOMR
output is the single x4 HOMR owner and is reused.

`restored_orchestrator.py` then writes a current-run inventory and calls
`reconstruct_dense_full_pipeline_route()`. Candidate/probe reconstruction and CNN scoring
operate on the current run; `historical_detector_artifact_runtime_input` is false for the
...
| `verified_source_page_worker` | disposable top-level Python worker per page | bounds lifetime of heavy detector source generation |
| pinned original HOMR | inside page worker | pinned-profile baseline only |
| `current_support_worker` | child worker | owns the x4 support contract |
| x4 SR / current HOMR / OMR-DLN | separate heavy child phases | prevents SR and HOMR peaks from being unnecessarily co-resident |
| dense route + CNN | imported/run after source workers exit | avoids retaining heavy x4 source-generation state |
...
`hybrid_results/`. Generated run outputs belong under ignored `logs/` paths unless a narrow
retention policy explicitly says otherwise.
...
See `docs/TWO_HOMR_MILESTONE.md` for the accepted accuracy/performance milestone and
