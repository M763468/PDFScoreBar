...
## この環境での `gemini` コマンド実行例（確認済み）

この環境では `gemini-cli` ではなく、実コマンド名は `gemini`。
`gemini --help` により、以下の形式が利用可能であることを確認済み。
...
# 非対話（ヘッドレス）で1回だけ相談
gemini -p "この設計案のリスクを3点挙げて"

# 初回プロンプトを渡して、そのまま対話継続
gemini -i "この差分を中間レビューして"
```

相談専用（編集させない前提を強める）:

```bash
# 読み取り/相談専用に寄せる（plan モード）
gemini --approval-mode plan -p "原因仮説を優先度順に5つ挙げて"
```
...
```bash
# JSON形式で受け取りたい場合
gemini -p "テスト観点を列挙して" -o json
```

運用メモ:

- まずは `--approval-mode plan` を優先し、相談用途に限定して使うと安全。
- 実装を委譲する場合は、書き込み担当（single writer）を明示してから実行する。
