...
- **lesson_017**: When a pipeline reconstructs page-local final numbering but MMR addresses global page/system/measure indices, carry score state only in the final presentation phase and persist the next logical number; never infer continuation from a legacy artifact's visible maximum because MMR skip state may be lost.
...
- **lesson_018**: Issue #294 worktree runs require local `data/evaluation2` images/fixtures and ignored detector weights. Shared `logs/`/`temp/` symlinks also require the manager worktree to be mounted at its absolute host path inside the profiling container; verify all inputs from inside `/workspace` before starting a fresh tag.
- **lesson_019**: Keep empty staff-mask components (for example scan borders) as extraction diagnostics, but do not treat them as semantic system-topology regressions when non-empty topology, numbering, and serialized measure semantics are unchanged.
