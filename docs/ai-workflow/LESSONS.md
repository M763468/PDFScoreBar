...
- **lesson_016**: Issue #294 worktree runs require local `data/evaluation2` images/fixtures and ignored detector weights. Shared `logs/`/`temp/` symlinks also require the manager worktree to be mounted at its absolute host path inside the profiling container; verify all inputs from inside `/workspace` before starting a fresh tag.
- **lesson_017**: Keep empty staff-mask components (for example scan borders) as extraction diagnostics, but do not treat them as semantic system-topology regressions when non-empty topology, numbering, and serialized measure semantics are unchanged.
