...
def load_gt_and_preds(gt_path, preds_path):
    ...
    if isinstance(gt_data, dict):
        if "predictions" in gt_data: gt_data = gt_data["predictions"]
...
def analyze_page(group, source, error_list, meta):
    ...
    if source == "local":
        ...
        if not gt_path.exists():
             ...
             if jsons: gt_path = sorted(jsons)[-1]

        preds_path = Path(f"logs/hybrid_generalization/eval2_{score}_{page_name}/expanded_candidates_nopeak.json")
    ...
    for err in fps:
        ...
        try:
            ...
            if idx >= len(fp_candidates): continue
...
def main():
    ...
    for err in errors:
        fname = err["filename"]
        if fname not in meta: continue
        info = meta[fname]
        if info["source"] == "deepscores": continue
    ...
    for (group, source), err_list in page_errors.items():
        ...
        if stats:
            print(f"{group:<40} | {stats['total_fp']:<10} | {stats['fp_ghost']:<8} | {stats['fp_near_hit']:<8} | {stats['total_fn']:<5}")
