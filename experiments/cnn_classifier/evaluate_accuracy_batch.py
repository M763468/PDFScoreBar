...
def main():
    ...
    with open(args.output_csv, "w", newline="") as f:
        writer = csv.DictWriter(
            f, fieldnames=["score", "page", "tp", "fp", "fn_total", "fn_cnn", "fn_det", "gt_count"]
        )
        writer.writeheader()
    ...
    # Aggregate
    agg = defaultdict(
        lambda: {"tp": 0, "fp": 0, "fn_total": 0, "fn_cnn": 0, "fn_det": 0, "gt": 0, "pages": 0}
    )
    for s in stats:
        bucket = agg[s["score"]]
        bucket["tp"] += s["tp"]
    ...
    print(
        f"{'Score':<35} | {'Pages':<5} | {'TP':<5} | {'FP':<5} | {'FN(Tot)':<8} | {'FN(CNN)':<8} | {'FN(Det)':<8} | {'Recall':<6} | {'Prec':<6}"
    )
    print("-" * 110)
    for score, data in agg.items():
        ...
        print(
            f"{score:<35} | {data['pages']:<5} | {tp:<5} | {fp:<5} | {fn:<8} | {data['fn_cnn']:<8} | {data['fn_det']:<8} | {recall:.1%} | {prec:.1%}"
        )
