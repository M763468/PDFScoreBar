...
def main():
    ...
    parser.add_argument(
        "--image-root", required=True, help="Root directory containing images (recursive search)"
    )
    parser.add_argument(
        "--json-root", required=True, help="Root directory containing hybrid_predictions.json files"
    )
    ...
    parser.add_argument(
        "--run-prefix",
        default="eval2",
        help="Only process run directories starting with this prefix",
    )
    parser.add_argument(
        "--skip-existing", action="store_true", help="Skip processing if output file already exists"
    )
    parser.add_argument(
        "--candidates-file",
        default="hybrid_predictions.json",
        help="Filename of candidate JSON in run dir",
    )
    ...
    for json_path in tqdm(json_files):
        ...
        results = process_page(
            image_path, candidates, model, cpu_transform, gpu_norm, args.threshold
        )
