...
def enhanced_analysis(metrics_path, stats_path, detections_path, image_path):
    ...
    for idx in fp_indices:
        ...
        print(
            f"FP #{idx:3d}: x={cx:5.1f} y={cy:5.1f} ({y_pct:4.1f}%) "
    ...
    for idx in [126, 217]:
        if idx in stats_by_idx:
            s = stats_by_idx[idx]
            print(
                f"FP #{idx}: ({s['x1']},{s['y1']})-({s['x2']},{s['y2']}) "
                f"W={s['width']} H={s['height']} Dist={s['dist']:.1f} Overlap={s['overlap']:.1f}"
            )
            print("  → This has Dist < 5, H < 24, W < 4, Overlap >= 5")
            print("  → Yet it's still a FP. Likely a stem that happens to meet all criteria.")
            print("  → Need additional discriminator (e.g., staff-crossing, vertical extent)")
