...
def analyze_gaps(metrics_path, gaps_path):
    ...
    for idx in tp_indices:
        ...
        g = gaps_by_idx[idx]
        dists = []
        if g["prev"] > 0: dists.append(g["prev"])
        if g["next"] > 0: dists.append(g["next"])
    ...
    for thresh in [2, 5, 8, 10, 12, 15, 20]:
        ...
        print(f"Threshold < {thresh:2d} px: Rejects {len(fps_rejected):2d} FPs, {len(tps_rejected):2d} TPs --> {status}")
