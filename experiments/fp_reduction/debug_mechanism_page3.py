...
def load_data(args):
    ...
    if img is None: raise FileNotFoundError(f"Image not found: {args.image}")
    
    # Load Preds
    with open(args.json) as f:
        data = json.load(f)
    if isinstance(data, dict): raw = data["predictions"]
    else: raw = data
    
    preds = []
    for item in raw:
        if isinstance(item, list): preds.append(item)
...
def main():
    ...
    for i, sys_indices in enumerate(valid_systems):
        ...
        for idx in sys_indices:
            ...
            if idx == sys_indices[0]: # Label first one
                cv2.putText(vis_b1, f"Sys {i}", (x1-30, int((y1+y2)/2)), cv2.FONT_HERSHEY_SIMPLEX, 0.8, col, 2)
    ...
    with open(report_path, "w") as f:
        ...
        f.write("- **Logic**: Reject if `abs(y_top - median_top) > 15` OR `abs(y_bot - median_bot) > 15`.\n")
