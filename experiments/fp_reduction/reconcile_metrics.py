...
def load_boxes(path, name):
    ...
    if isinstance(data, dict):
        if "predictions" in data: raw = data["predictions"]
        elif "annotations" in data: raw = data["annotations"] # COCO style?
        elif "boxes" in data: raw = data["boxes"]
    ...
    for item in raw:
        if isinstance(item, list):
            ...
            # Try various keys
            if "barline_location" in item: boxes.append(item["barline_location"])
            elif "orig_bbox" in item: boxes.append(item["orig_bbox"])
            elif "bbox" in item: boxes.append(item["bbox"])
            elif "pred_bbox" in item: boxes.append(item["pred_bbox"])
    
    arr = np.array(boxes).astype(float)
    if len(arr) > 0:
        print(f"  {name}: {len(arr)} boxes. Sample: {arr[0]}")
        print(f"  {name} Range: X[{arr[:,0].min():.1f}, {arr[:,2].max():.1f}] Y[{arr[:,1].min():.1f}, {arr[:,3].max():.1f}]")
    else:
...
def main():
    ...
    for i, g in enumerate(gt):
        ...
        if 0.1 < best_iou < 0.5:
            if len(example_failures) < 10:
                example_failures.append({"gt": g.tolist(), "pred": preds[best_p_idx].tolist(), "iou": best_iou})
    ...
    if example_failures:
        ...
        for idx, item in enumerate(example_failures):
            ...
            # Draw (translated)
            cv2.rectangle(crop, (int(g[0]-x1), int(g[1]-y1)), (int(g[2]-x1), int(g[3]-y1)), (0,255,0), 1) # GT
            cv2.rectangle(crop, (int(p[0]-x1), int(p[1]-y1)), (int(p[2]-x1), int(p[3]-y1)), (0,0,255), 1) # Pred
            
            cv2.putText(crop, f"IoU={item['iou']:.2f}", (5, 15), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255,255,255), 2)
            cv2.putText(crop, f"IoU={item['iou']:.2f}", (5, 15), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,0,0), 1)
