...
def create_debug_overlay(
    ...
    cv2.putText(
        img,
        f"Tol: top={tol_top:.1f}px, bot={tol_bottom:.1f}px",
        (10, 30),
        cv2.FONT_HERSHEY_SIMPLEX,
        0.8,
        (255, 255, 255),
...
def main():
    ...
    for tol in absolute_tolerances:
        ...
        print(
            f"  Tol={tol}px: TP={metrics['TP']}, FP={metrics['FP']}, FN={metrics['FN']}, "
    ...
    for ratio in ratio_tolerances:
        ...
        print(
            f"  Ratio={ratio} ({tol_px:.1f}px): TP={metrics['TP']}, FP={metrics['FP']}, "
    ...
    with open(csv_path, "w") as f:
        ...
        for r in results:
            ...
            f.write(
                f"{r['mode']},{r['tol_top_px']:.2f},{r['tol_bottom_px']:.2f},"
    ...
    with open(md_path, "w") as f:
        ...
        for r in results:
            if r["mode"] == "absolute":
                f.write(
                    f"| {r['tol_top_px']:.0f} | {r['TP']} | {r['FP']} | {r['FN']} | "
        ...
        for r in results:
            if r["mode"] == "ratio":
                f.write(
                    f"| {r['tol_ratio']:.1f} | {r['tol_top_px']:.1f} | {r['TP']} | {r['FP']} | "
    ...
    for idx, (r, fp, tp_loss) in enumerate(top_3):
        ...
        create_debug_overlay(
            preds_list, rows, accepted, noise_indices, args.image, output_path, tol_top, tol_bottom
        )
