...
def load_boxes(path, is_gt=False):
    ...
    boxes = []
    if is_gt:
        if isinstance(data, list) and len(data) > 0 and isinstance(data[0], dict) and "barline_location" in data[0]:
...
def main():
    ...
    # Overlays
    # 1. GT + Original Preds
    draw_overlay(img, preds_orig, gt, 
                 os.path.join(args.output, "OVERLAY_GT_OriginalPreds.jpg"), 
                 "Original")
