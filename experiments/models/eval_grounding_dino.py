...
def parse_args():
    ...
    default_config = str(
        repo_root / "external/grounding_dino/groundingdino/config/GroundingDINO_SwinT_OGC.py"
    )
    default_weights = str(repo_root / "external/grounding_dino/weights/groundingdino_swint_ogc.pth")

    parser.add_argument(
        "--config", type=str, default=default_config, help="Path to GroundingDINO config file"
    )
    parser.add_argument(
        "--weights", type=str, default=default_weights, help="Path to GroundingDINO weights file"
    )
