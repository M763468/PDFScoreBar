# load_json()

> God node · 41 connections · `src/pipeline/utils/io.py`

**Community:** [load_json](load_json.md)

## Connections by Relation

### calls
- .run() `EXTRACTED`
- run() `EXTRACTED`
- [main()](main%28%29.md) `EXTRACTED`
- build_page_specs() `EXTRACTED`
- .run_base_numbering_and_barline_correction() `EXTRACTED`
- .run_final_numbering_and_overlays() `EXTRACTED`
- run() `EXTRACTED`
- build_mmr_support() `EXTRACTED`
- [main()](main%28%29.md) `EXTRACTED`
- _representative_report() `EXTRACTED`
- .run_mmr_batch_detection() `EXTRACTED`
- build_numbering() `EXTRACTED`
- test_three_page_phase_c_preserves_manual_precedence_without_cross_page_leakage() `EXTRACTED`
- augment_report() `EXTRACTED`
- _preflight() `EXTRACTED`
- _geometry_report() `EXTRACTED`
- [main()](main%28%29.md) `EXTRACTED`
- [main()](main%28%29.md) `EXTRACTED`
- .test_full_sequence() `EXTRACTED`
- _load_expected() `EXTRACTED`

### contains
- io.py `EXTRACTED`

### imports
- [pipeline/orchestrator.py](pipeline-orchestrator.py.md) `EXTRACTED`
- [run_phase_c_mmr_regression.py](run_phase_c_mmr_regression.py.md) `EXTRACTED`
- run_full68_mmr_reuse.py `EXTRACTED`
- [run_original_geometry_graft.py](run_original_geometry_graft.py.md) `EXTRACTED`
- [phase_b_page001_acceptance.py](phase_b_page001_acceptance.py.md) `EXTRACTED`
- test_issue257_mmr_page_rebase.py `EXTRACTED`
- run_representative_mmr_reuse.py `EXTRACTED`
- [mmr_support_reuse.py](mmr_support_reuse.py.md) `EXTRACTED`
- [phase_c_phase_a_support.py](phase_c_phase_a_support.py.md) `EXTRACTED`
- validate_mmr_support_mapping.py `EXTRACTED`
- run_phase_b_page001_acceptance.py `EXTRACTED`
- test_pipeline_integration.py `EXTRACTED`
- [test_issue264_phase_b_manifest_discovery.py](test_issue264_phase_b_manifest_discovery.py.md) `EXTRACTED`

### references
- Path `EXTRACTED`
- [Any](Any.md) `EXTRACTED`

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*