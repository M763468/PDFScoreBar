# run_full_pipeline.py

> 36 nodes · cohesion 0.17

## Key Concepts

- **run_full_pipeline.py** (32 connections) — `tools/run_full_pipeline.py`
- **main()** (26 connections) — `tools/run_full_pipeline.py`
- **Any** (20 connections)
- **Path** (20 connections)
- **_get_nested()** (12 connections) — `tools/run_full_pipeline.py`
- **_resolve_page_filters()** (7 connections) — `tools/run_full_pipeline.py`
- **_build_pdf_command()** (6 connections) — `tools/run_full_pipeline.py`
- **_resolve_barlines_and_masks_config()** (6 connections) — `tools/run_full_pipeline.py`
- **_run_detection_step()** (6 connections) — `tools/run_full_pipeline.py`
- **_collect_images()** (5 connections) — `tools/run_full_pipeline.py`
- **_empty_numbering_payload()** (5 connections) — `tools/run_full_pipeline.py`
- **_is_blank_page()** (5 connections) — `tools/run_full_pipeline.py`
- **_resolve_page_ids()** (5 connections) — `tools/run_full_pipeline.py`
- **_staff_detect_failed()** (5 connections) — `tools/run_full_pipeline.py`
- **_apply_barline_overrides()** (4 connections) — `tools/run_full_pipeline.py`
- **_build_manifest()** (4 connections) — `tools/run_full_pipeline.py`
- **_ensure_dir()** (4 connections) — `tools/run_full_pipeline.py`
- **_get_user_exclude_indices()** (4 connections) — `tools/run_full_pipeline.py`
- **_load_json()** (4 connections) — `tools/run_full_pipeline.py`
- **_load_yaml()** (4 connections) — `tools/run_full_pipeline.py`
- **_resolve_page_runs()** (4 connections) — `tools/run_full_pipeline.py`
- **_resolve_paths_from_detection()** (4 connections) — `tools/run_full_pipeline.py`
- **_write_json()** (4 connections) — `tools/run_full_pipeline.py`
- **_write_manifest()** (4 connections) — `tools/run_full_pipeline.py`
- **_build_add_measure_numbers_cmd()** (3 connections) — `tools/run_full_pipeline.py`
- *... and 11 more nodes in this community*

## Relationships

- [run_detection_step](run_detection_step.md) (2 shared connections)
- [barline_iou](barline_iou.md) (2 shared connections)
- [barline_evaluation.py](barline_evaluation.py.md) (1 shared connections)

## Source Files

- `tools/run_full_pipeline.py`

## Audit Trail

- EXTRACTED: 115 (100%)
- INFERRED: 0 (0%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*