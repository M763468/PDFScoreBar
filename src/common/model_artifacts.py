...
class ModelArtifactManifest:
    """Validated fields required to locate and verify one production model artifact."""
...
def load_model_artifact_manifest(path: Path | str) -> ModelArtifactManifest:
    ...
    if not isinstance(release, dict):
        raise ModelArtifactManifestError(
            f"Manifest {manifest_path} requires object field 'release'"
        )
    ...
    *,
    ...
    try:
        ...
        raise ModelArtifactMissingError(
            f"{exc}. Materialize it explicitly with: "
            f"python -m src.common.model_artifacts materialize {manifest.path}"
        ) from exc
