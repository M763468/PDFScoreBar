"""Run the pinned maintained HOMR runtime on one original page.

The current support worker is intentionally x4-only and converts its output
back to original coordinates.  The authoritative maintained baseline instead
consumes the original page with ``sr_scale=1`` and emits the same detector
artifact contract as the historical profile.
"""
...
EXPECTED_HOMR_COMMIT = "457e7c6518a10ba755db2e60883419e56c4d7369"
...
def _homr_installed_commit(direct_url: dict[str, Any] | None) -> str | None:
    if not isinstance(direct_url, dict) or not isinstance(direct_url.get("vcs_info"), dict):
        return None
...
def run(image: Path, output_root: Path, result_path: Path) -> dict[str, Any]:
    ...
    if image_run_dir.exists() or result_path.exists():
        raise FileExistsError(image_run_dir if image_run_dir.exists() else result_path)
    ...
    installed_commit = _homr_installed_commit(direct_url)
    if installed_commit != EXPECTED_HOMR_COMMIT:
        raise RuntimeError(
            "Installed HOMR direct_url commit does not match the immutable profile: "
            f"expected={EXPECTED_HOMR_COMMIT} actual={installed_commit}"
        )
...
def main() -> int:
    ...
    try:
        ...
        return 1
    print(json.dumps({"status": payload["status"], "result": str(args.result.resolve())}))
    return 0
