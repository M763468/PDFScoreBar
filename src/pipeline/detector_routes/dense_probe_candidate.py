...
def _reject_removed_keys(config: dict[str, Any], removed: dict[str, str], *, section: str) -> None:
    for old_key, new_key in removed.items():
        if old_key in config:
            raise ValueError(
                f"Removed dense probe-candidate route key `{old_key}` in {section}. "
...
def validate_complete_contract(eval_output_dir: Path) -> None:
    ...
    if expected != expected_count or evaluated != expected_count or missing:
        raise RuntimeError(
            ...
            f"expected_pages={expected} evaluated_pages={evaluated} expected_count={expected_count} missing_pages={len(missing)}"
        )
...
def config_from_yaml(path: Path) -> DenseProbeCandidateConfig:
    ...
    _reject_removed_keys(route, REMOVED_CONFIG_KEYS, section="dense_probe_candidate_route")
