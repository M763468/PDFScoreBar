...
def is_in_container() -> bool:
    ...
    return Path("/.dockerenv").exists() or (Path("/workspace").exists() and Path("/opt/venv_sr").exists())
...
def get_docker_exec_prefix() -> List[str]:
    ...
    try:
        ...
        result = subprocess.run(
            ["docker", "ps", "--filter", "name=sr_eval_gpu", "--filter", "status=running", "--format", "{{.Names}}"],
            capture_output=True,
            text=True,
            check=False
        )
        if "sr_eval_gpu" in result.stdout:
            # We set PYTHONPATH to /workspace which is where the volume is mounted inside the container
            return [
                "docker", "exec", 
                "-w", "/workspace", 
                "-e", "PYTHONPATH=/workspace:/workspace/external/homr",
                "sr_eval_gpu"
            ]
    except Exception:
        pass
...
def get_pipeline_python(step_name: Optional[str] = None) -> List[str]:
    ...
    Args:
        step_name: Optional step name (e.g., 'detection', 'homr', 'omr_dln', 'sr', 'pdf_to_images')
    
    Order of preference for heavy steps (detection/homr/omr_dln/sr):
    1. If on host and sr_eval_gpu is running: 'docker exec sr_eval_gpu /opt/venv_sr/bin/python'.
    2. If in container: /opt/venv_sr/bin/python.
    3. PIPELINE_PYTHON environment variable.
    4. Fallback to .venv_pdf/bin/python or current sys.executable.
    """
    env_python = os.environ.get("PIPELINE_PYTHON")
    
    # Check for heavy steps first
    ...
    # 1. If PIPELINE_PYTHON is explicitly set and we didn't use a heavy-step override
    if env_python:
        return [env_python]

    # 2. If already in container but not a heavy step (or /opt/venv_sr missing)
    if is_in_container():
        return [sys.executable]

    # 3. Default host fallback for light steps
