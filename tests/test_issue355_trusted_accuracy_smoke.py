...
def test_standard_smoke_uses_production_detector_contract():
    ...
    assert _without(smoke["detection"], ignored) == _without(
        production["detection"], ignored
    )
...
def test_standard_smoke_uses_accepted_evaluation_input_contract():
    ...
    assert smoke["inputs"]["pdf_path"] == (
        "data/evaluation2/pdfs/Va_Prokofiev_Symphony1.pdf"
    )
...
def test_standard_smoke_runs_post_pipeline_accuracy_gate():
    runtime_script = (ROOT / "scripts/docker_runtime_validation.sh").read_text(
        encoding="utf-8"
    )
    makefile = (ROOT / "Makefile").read_text(encoding="utf-8")

    assert "tools/verification/verify_detector_accuracy_smoke.py" in runtime_script
    assert 'smoke_run_id="smoke_test_detection_$(date -u +%Y%m%dT%H%M%SZ)"' in (
        runtime_script
    )
