...
def main():
    ...
    baseline_final_path = REPO_ROOT / "logs/phase4_notehead_geom/20251218_page3_hybrid_tol5_geom/filtered_barlines.json"
    ...
    print(f"{'GT Index':<10} | {'In Merge?':<10} | {'Merge IoU':<10} | {'GT Bbox':<25} | {'Merge Bbox'}")
    print("-" * 80)
    for row in results:
        print(f"{row['gt_index']:<10} | {row['in_s2_merge_output']:<10} | {row['s2_merge_iou']:<10} | {str(row['gt_box']):<25} | {row['s2_merge_box']}")
