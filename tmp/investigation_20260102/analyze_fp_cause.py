...
def analyze_fp(page_dir):
    ...
    for i, fp_box in enumerate(fp_boxes):
        ...
        if best_match:
            ...
            print(
                f"  Ratios: Peak={best_match.get('peak_ratio')}, Top={best_match.get('top_ratio')}, Bottom={best_match.get('bottom_ratio')}"
            )
            print(
                f"  Extend Ratios: Top={best_match.get('extended_top_max_ratio')}, Bottom={best_match.get('extended_bottom_max_ratio')}"
            )
