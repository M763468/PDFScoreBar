...
def main():
    ...
    for sys_idx, system in enumerate(page["systems"]):
        for staff in system["staves"]:
            ...
            all_staves.append({
                "sys_group": sys_idx,
                "bbox": staff["bbox"],
                "measures": system["measures"] # These belong to snake-wise flow but roughly align strictly vertical for systems
            })
    ...
    for i in range(len(all_staves) - 1):
        ...
        print(f"Staff {i+1} -> {i+2}: Gap={gap} (Thresh={dist_threshold:.2f}) -> DistCheck={gap_judge}{status}")
