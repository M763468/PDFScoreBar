...
def main():
    ...
    parser.add_argument(
        "--target", action="append", help="Format: work:page:measure_index (e.g. Sibelius:3:22)"
    )
    parser.add_argument(
        "--output-dir", type=Path, default=Path("logs/experiments/failure_analysis_v2")
    )
    ...
    parser.add_argument(
        "--numbering-root",
        type=Path,
        default=Path("logs/experiments/global_mmr_eval_v3_ocr_fix_crop"),
    )
    ...
    for t in args.target:
        ...
        if len(parts) == 3:
            ...
            work_low = work.lower()
            if "sibelius" in work_low:
                work = "Sibelius-Violin_Concerto-Viola"
            elif "prokofiev5" in work_low or "prok5" in work_low:
                work = "prokofiev5"
            elif "prok1" in work_low or "prokofiev1" in work_low:
                work = "Va_Prokofiev_Symphony1"
            elif "festival" in work_low:
                work = "Shostakovich-Festival_Overture_Va"
            elif "sym5" in work_low:
                work = "Shosrakovich-Sym5-Va"
    ...
    for work, page_num, target_m_idx in targets:
        ...
        num_json = (
            args.numbering_root / work / page_str / "numbering_initial.json"
        )  # Use initial to get pure bboxes
        ...
        if has_hbar:
            ...
            cv2.line(
                context_img, (0, hcy_global), (context_img.shape[1], hcy_global), (0, 0, 255), 1
            )
        ...
        if ocr_res:
            ...
            for item in ocr_res:
                ...
                print(
                    f"    OCR: '{text}' (conf={score:.2f}, dx={dist_x_norm:.2f}, dy={dist_y_norm:.2f}{hbar_dist_str})"
                )
