...
def analyze_deepscores_components(
    ...
    print(f"Passed filters (TP candidates): {passed_comps} ({passed_comps/total_comps*100:.1f}%)")
    print("-" * 30)
    print("Filtered out:")
    print(f"  Area < {min_area}: {filtered_by_area} ({filtered_by_area/total_comps*100:.1f}%)")
    print(f"  Height < {min_height}: {filtered_by_height} ({filtered_by_height/total_comps*100:.1f}%)")
    print(f"  H/W Ratio < {vertical_ratio}: {filtered_by_ratio} ({filtered_by_ratio/total_comps*100:.1f}%)")
    print("-" * 30)
