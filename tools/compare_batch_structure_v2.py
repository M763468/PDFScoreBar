...
def compare_batches(old_root, new_root, config_paths):
    ...
    for config_path in config_paths:
        if not Path(config_path).exists(): continue
        ...
        for entry in pages:
            ...
            old_path = Path(old_rel).resolve() if Path(old_rel).is_absolute() else (REPO_ROOT / old_rel).resolve()
