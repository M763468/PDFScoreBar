...
def debug_mask_alignment(image_path, mask_path, numbering_json_path, output_path, target_measure_idx=None):
    """
    ...
    for i, m in enumerate(measures_to_draw):
        ...
        if mx2 > mx1 and my2 > my1:
            ...
            if target_w > 0 and target_h > 0:
                resized_mask_patch = cv2.resize(mask_patch, (target_w, target_h), interpolation=cv2.INTER_NEAREST)
                ...
                if h_match > 0 and w_match > 0:
                    blend = cv2.addWeighted(roi[:h_match, :w_match], 0.5, resized_mask_patch[:h_match, :w_match], 0.5, 0)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--measure-idx", type=int, default=None, help="Specific measure index to visualize")
