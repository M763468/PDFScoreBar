...
def main():
    ...
    parser.add_argument(
        "--force-measure", type=int, nargs="+", help="Force OCR on these measure numbers"
    )
    ...
    for page in data["pages"]:
        for system in page["systems"]:
            for measure in system["measures"]:
                ...
                if force_check:
                    print(
                        f"  [FORCE] M{m_num} Density Check: Px={pixel_count} (Limit={args.threshold})"
                    )
                ...
                if pixel_count <= args.threshold or force_check:
                    ...
                    if has_hbar or force_check:
                        ...
                        if force_check:
                            debug_ocr_path = (
                                Path(args.output_image).parent / f"roi_M{m_num}_ocr.png"
                            )
                        ...
                        cv2.rectangle(
                            vis_img, (roi_x1, roi_y1_ocr), (roi_x2, roi_y2_ocr), (0, 0, 255), 2
                        )  # Red
                            ...
                            try:
                                ...
                                if ocr_result:
                                    ...
                                    for res in ocr_result:
                                        ...
                                        if force_check:
                                            print(
                                                f"    [OCR] '{text}' score={score:.2f} dist={dist:.1f} Y={text_center_y:.1f} (StaffTop={staff_top_y}) Inside={is_inside_staff}"
                                            )
                                    ...
                                    if number:
                                        ...
                                        color = (0, 255, 0)  # Green
                                        print(
                                            f"  [DEBUG] M{m_num} FOUND: {number} | HBar={has_hbar}"
                                        )
                                    else:
                                        status_text = f"REJ: '{full_valid_text}' (hbar={has_hbar})"
                                        color = (0, 165, 255)  # Orange
                                        print(
                                            f"  [DEBUG] M{m_num} REJECTED TEXT: '{full_valid_text}' | HBar={has_hbar}"
                                        )
                                else:
                        ...
                        cv2.rectangle(
                            vis_img,
                            (roi_x1, roi_y1_ocr - th - 5),
                            (roi_x1 + tw, roi_y1_ocr),
                            (255, 255, 255),
                            -1,
                        )
                        cv2.putText(
