...
def offset_boxes(boxes: List[Box], x_offset: int, y_offset: int) -> List[Box]:
    shifted = []
    for x1, y1, x2, y2 in boxes:
        shifted.append((x1 + x_offset, y1 + y_offset, x2 + x_offset, y2 + y_offset))
    return shifted
...
def main() -> None:
    ...
    ap.add_argument("--crop-left", type=Path, help="Left crop detections.json")
    ap.add_argument("--crop-right", type=Path, help="Right crop detections.json")
    ap.add_argument("--left-offset", type=int, default=0)
    ap.add_argument("--right-offset", type=int, default=0)
    ...
    merged = list(base_boxes)

    if args.crop_left and args.crop_left.exists():
        merged.extend(offset_boxes(load_homr_boxes(args.crop_left), args.left_offset, 0))
    if args.crop_right and args.crop_right.exists():
        merged.extend(offset_boxes(load_homr_boxes(args.crop_right), args.right_offset, 0))
