...
def eval_pdf(pdf_name):
    ...
    for page_dir in sorted(probe_scan_dir.iterdir()):
        ...
        match_result = greedy_barline_match(pred_bboxes, gt_bboxes, rule_name="baseline_iou", iou_threshold=0.5)
...
def main():
    ...
    for pdf in PDFS:
        ...
        if stats:
            ...
            print(f"  Result: TP={pdf_tp}, FP={pdf_fp}, FN={pdf_fn}, Recall={recall:.1%}, Prec={prec:.1%}")
            overall.append({
                "name": pdf,
                "tp": pdf_tp,
                "fp": pdf_fp,
    ...
    for o in overall:
        ...
        print(f"{o['name']:<45} | {o['tp']:<5} | {o['fp']:<5} | {o['fn']:<5} | {recall:>7.1%} | {prec:>7.1%}")
    
    g_recall = total_tp / total_gt if total_gt > 0 else 0
    g_prec = total_tp / (total_tp + total_fp) if (total_tp + total_fp) > 0 else 0
    print("-"*90)
    print(f"{'GLOBAL TOTAL':<45} | {total_tp:<5} | {total_fp:<5} | {total_fn:<5} | {g_recall:>7.2%} | {g_prec:>7.2%}")
