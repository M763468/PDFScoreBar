...
def main():
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("debug_outputs/failure_visualizations_v9"))
    ...
    for subdir in tqdm(subdirs, desc="Scores"):
        ...
        for page_dir in page_dirs:
            ...
            for fp_idx in res.false_positive_indices:
                ...
                cv2.putText(crop, f"ERR: FP (Score: {fp_score:.3f})", (10, 30), 0, 0.6, (0, 165, 255), 2)
                cv2.imwrite(str(args.output_dir / f"{label}.png"), crop)
                global_counter += 1
