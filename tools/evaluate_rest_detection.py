...
def evaluate_rest_detection(eval_root, overrides_root):
    ...
    det_prec = (
        total_det_tp / (total_det_tp + total_det_fp) if (total_det_tp + total_det_fp) > 0 else 0
    )
    det_rec = total_det_tp / total_gt_rests if total_gt_rests > 0 else 0

    pipe_prec = (
        total_pipeline_tp / (total_pipeline_tp + total_pipeline_fp)
        if (total_pipeline_tp + total_pipeline_fp) > 0
        else 0
    )
    ...
    print("\n--- Summary Metrics ---")
    print("Stage 1: Candidate Detection (Classifier)")
    print(f"  Precision: {det_prec:.4f}")
    print(f"  Recall:    {det_rec:.4f} ({total_det_tp}/{total_gt_rests})")
    print("Stage 2: Full Pipeline (Candidate + OCR)")
