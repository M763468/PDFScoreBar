...
def main():
    ...
    for b in raw_bars:
        ...
        if isinstance(b, list):
            bbox = b
        elif "barline_location" in b:
            bbox = b["barline_location"]
        elif "bbox" in b:
            bbox = b["bbox"]
    ...
    for i in range(len(bands) - 1):
        ...
        for j in range(1, num_labels):
            ...
            if gh > gap_h * 0.8:
                ...
                # Draw on debug
                cv2.rectangle(
                    debug_img, (gx, y1_bot + gy), (gx + gw, y1_bot + gy + gh), (0, 0, 255), 2
                )

        print(
            f"Pair {i}({y1_bot}) -> {i + 1}({y2_top}): Gap={gap_h}px. Detected Connectors={connected_barlines}"
        )
        ...
        cv2.putText(
            debug_img,
            f"Gap {gap_h}px, Conn={connected_barlines}",
            (50, (y1_bot + y2_top) // 2),
