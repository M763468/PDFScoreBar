...
def export_crops(work, page, target_measure, output_dir):
    ...
    # Paths
    json_path = Path(f"logs/experiments/batch_verification_20260107_v5/{work}/{page_str}/numbering_final.json")
    if not json_path.exists():
        print(f"  [Error] JSON not found: {json_path}")
        return

    img_path = Path(f"data/evaluation2/images/{work}/{page_str}.png")
    mask_path = Path(f"logs/hybrid_generalization/eval2_{work}_{page_str}/baseline/{page_str}/{page_str}/{page_str}_debug_6_notehead.png")
        ...
        for system in page_data["systems"]:
            for measure in system["measures"]:
                ...
                if abs(m_num - target_measure) <= 2:
                    ...
                    cx2 = min(img.shape[1], x2 + pad); cy2 = min(img.shape[0], y2 + pad)
                    context_crop = img[cy1:cy2, cx1:cx2].copy()
                    
                    # Draw ROI indicators on context
                    # Blue: Measure BBox
                    cv2.rectangle(context_crop, (x1-cx1, y1-cy1), (x2-cx1, y2-cy1), (255, 0, 0), 2)
                    
                    crop_name = f"{work}_{page_str}_M{m_num}_context.png"
                    cv2.imwrite(str(output_dir / crop_name), context_crop)
                    
                    # 2. OCR ROI (80px margin)
                    ry1 = max(0, y1 - 80); ry2 = min(img.shape[0], y2 + 30)
                    rx1 = max(0, x1 - 10); rx2 = min(img.shape[1], x2 + 10)
                    ocr_crop = img[ry1:ry2, rx1:rx2]
                    cv2.imwrite(str(output_dir / f"{work}_{page_str}_M{m_num}_ocr_roi.png"), ocr_crop)
                    ...
                    mask_crop = mask[max(0, my1-20):min(mask.shape[0], my2+20), max(0, mx1-20):min(mask.shape[1], mx2+20)]
                    cv2.imwrite(str(output_dir / f"{work}_{page_str}_M{m_num}_mask.png"), mask_crop)
                    
                    if m_num == target_measure: found = True
...
def main():
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("logs/experiments/failure_crops_20260108"))
