...
def analyze_crossings(metrics_path, stats_path):
    ...
    for crossings in sorted(tp_counter.keys()):
        print(f"  {crossings} crossings: {tp_counter[crossings]:3d} TPs ({tp_counter[crossings]/len(tp_crossings)*100:5.1f}%)")
    
    print("\nFalse Positives (Stems/Artifacts):")
    for crossings in sorted(fp_counter.keys()):
        print(f"  {crossings} crossings: {fp_counter[crossings]:3d} FPs ({fp_counter[crossings]/len(fp_crossings)*100:5.1f}%)")
    ...
    for threshold in [1, 2, 3, 4, 5]:
        rejected_tps = [i for i in tp_indices if i in stats_by_idx and stats_by_idx[i]["crossings"] < threshold]
        rejected_fps = [i for i in fp_indices if i in stats_by_idx and stats_by_idx[i]["crossings"] < threshold]
        
        if len(rejected_tps) == 0:
            print(f"✓ SAFE: crossings < {threshold} → Rejects {len(rejected_fps)} FPs, 0 TPs")
        else:
            print(f"✗ UNSAFE: crossings < {threshold} → Rejects {len(rejected_fps)} FPs, {len(rejected_tps)} TPs")
    ...
    low_crossing_fps = [i for i in fp_indices if i in stats_by_idx and stats_by_idx[i]["crossings"] < 3]
    ...
    print(f"  - High overlap (>= 5px) AND low crossing: {len(low_crossing_fps) - len(low_overlap_and_low_crossing)} FPs")
    
    print("\n" + "=" * 80)
    print("EXPECTED EFFECT SIZE")
    print("=" * 80)
    print(f"If we apply: REJECT if (crossings < 3) AND (overlap < 5):")
    print(f"  - FPs rejected: {len(low_overlap_and_low_crossing)}")
    print(f"  - TPs rejected: 0 (verified above)")
