...
def analyze(metrics_path, stats_path):
    ...
    fp_indices = set(page_metrics["false_positives_indices"]) if "false_positives_indices" in page_metrics else set()
    ...
    print(f"TPs close to notehead: {len(tp_close)} / {len(tp_stats)} ({len(tp_close)/len(tp_stats)*100:.1f}%)")
    print(f"FPs close to notehead: {len(fp_close)} / {len(fp_stats)} ({len(fp_close)/len(fp_stats)*100:.1f}%)")
    ...
    for h_thresh in [20, 22, 24, 25, 26, 28, 30]:
        for w_thresh in [2, 3, 4, 5]:
             for o_thresh in [0, 1, 5, 10]:
                ...
                rejected_tps = [x for x in tp_close if x["height"] < h_thresh and x["width"] < w_thresh and x["overlap"] >= o_thresh]
                rejected_fps = [x for x in fp_close if x["height"] < h_thresh and x["width"] < w_thresh and x["overlap"] >= o_thresh]
                
                if len(rejected_tps) == 0 and len(rejected_fps) > 0:
                     print(f"SAFE FILTER: H < {h_thresh} AND W < {w_thresh} AND Overlap >= {o_thresh} -> Rejects {len(rejected_fps)} FPs")
