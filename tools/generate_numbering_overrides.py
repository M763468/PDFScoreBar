...
def check_musical_elements(roi_img, roi_staff_mask, roi_notehead_mask, hbar_rect, text_boxes):
    ...
    """
    debug_masks = {}
    if roi_img is None: return False, "No Image", {}
    ...
    if hbar_rect:
        ...
        margin = 3
        cv2.rectangle(exclusion_mask, (max(0, x-margin), max(0, y-margin)), (min(w, x+w_rect+margin), min(h, y+h_rect+margin)), 255, -1)
...
def visualize_masks(debug_img, roi_rect, masks):
    ...
    """
    if debug_img is None or not masks: return
    ...
    if roi_slice.shape[:2] != (h, w): return # Safety check
        ...
        for sys_idx, system in enumerate(page["systems"]):
            for m_idx, measure in enumerate(system["measures"]):
                ...
                if not is_hbar:
                    ...
                    print(f"  [INFO] P{page_num} S{sys_idx} M{m_num}: H-Bar Found: w={hw}, h={hh}, AR={hw/hh:.2f}")
                    ...
                    if ocr_result:
                        ...
                        for res in ocr_result:
                            ...
                            if score < 0.6:
                                ...
                                draw_debug_info(debug_img, abs_x1, abs_y1, abs_x2, abs_y2, 'rejected', text, f"Score:{score:.2f}")
                                continue
                        ...
                        if valid_texts:
                            ...
                            if number:
                                ...
                                if has_elements:
                                    print(f"  [REJECT] P{page_num} S{sys_idx} M{m_num}: Text='{full_text}' -> Musical Elements Found: {reason}")
                                    draw_debug_info(debug_img, x1, y1, x2, y2, 'rejected', f"{number}", reason)
                                else:
