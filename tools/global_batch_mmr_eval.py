...
def process_page(
    ...
    cmd_2 = [
        ...
        str(threshold),
        "--rescue-threshold",
        str(rescue_threshold),
    ]
    if model_path:
        cmd_2.extend(["--model-path", str(model_path)])

    if not run_command(cmd_2, "Generate Overrides"):
        return False
    ...
    if not run_command(cmd_3, "Final Numbering"):
        return False
...
def main():
    ...
    parser.add_argument(
        "--output-dir", type=Path, default=Path("logs/experiments/global_mmr_eval_v1")
    )
    parser.add_argument("--threshold", type=float, default=0.5)
    parser.add_argument("--rescue-threshold", type=float, default=0.1)
    parser.add_argument("--model-path", type=Path, default=None)
    parser.add_argument(
        "--filter", type=str, default=None, help="Filter works by name (case-insensitive)"
    )
    ...
    for work in sorted(works):
        ...
        img_work_name = work
        if work == "Va_Prokofiev_Symphony1":
            img_work_name = "prokofiev1"  # Typical mapping
        ...
        pages = sorted(
            [d.name for d in work_ann_dir.iterdir() if d.is_dir() and d.name.startswith("page_")]
        )
