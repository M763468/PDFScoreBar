#!/usr/bin/env python3
"""Regenerate and compare all 68 baseline HOMR pages with the verified fresh route."""
...
EXCLUDED_STEM_MARKERS = ("_teaser", "_debug", "_staff", "_tesseract", "_proxy")
...
def discover_canonical_images(image_root: Path) -> list[Path]:
    images = [
        ...
        and not any(marker in path.stem.lower() for marker in EXCLUDED_STEM_MARKERS)
    ]
...
def main() -> int:
    ...
    report: dict[str, Any] = {
        "schema_version": "issue245.fresh_upstream_full68_probe.v1",
        "status": "preflight_completed" if args.preflight_only else "running",
        ...
        try:
            ...
            if args.resume and existing and existing.get("status") == "completed":
                ...
                if existing_path.is_file():
                    ...
                    page_result["inference_reused"] = True
            if candidate is None:
                ...
                page_result["inference_reused"] = False

            comparison = compare_records(
                load_records(historical_detection), load_records(candidate)
            )
            page_result.update(
                {
                    "status": "completed",
