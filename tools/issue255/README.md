...
## 1. Focused canonical fresh run

`run_focused_fresh_detector.py` loads `configs/dense_full_pipeline.yaml` and preserves
its `detection` and `steps` mappings. It changes only the selected image directory,
image glob, run ID and output root.
...
Use another new run ID for:

```text
data/evaluation2/images/Shostakovich-Sym5-Va/page_014.png
```

The wrapper fails unless the detector input contract is exactly:
...
Each run writes `issue255_focused_fresh_run_contract.json`. It records repository and
runtime identity, canonical/effective config hashes, coordinate spaces and hashes for
the generated baseline HOMR, SR HOMR, OMR-DLN, hybrid, probe and CNN artifacts.
Do not use `--skip-existing`; each focused run requires a new output identity.
