...
def _historical_components(
    ...
    for stage in STAGES:
        ...
        if stage_record is None:
            raise ValueError(
                f"Expected one historical {stage} artifact for {label}: {run_dir}"
            )
    ...
    return [
        {
            ...
            "counterpart_component_matches": _stage_provenance(
                box, counterpart_components
            ),
        }
        for box in boxes
    ]
...
def build_report(run_root: Path, historical_comparison: Path) -> dict[str, Any]:
    ...
    if not all(
        ...
        for value in (public_pages, row_pages, historical_pages)
    ):
    ...
    for label, row_page in row_pages.items():
        ...
        if not isinstance(public_page, Mapping) or not isinstance(
            historical_page, Mapping
        ):
        ...
        public_paths, public_components = _public_components(
            artifacts, str(label)
        )
        comparisons = {
            stage: _component_comparison(
                historical_components[stage], public_components[stage]
            )
            for stage in STAGES
        }
        differing_components = [
            stage for stage in STAGES if not comparisons[stage]["exact_match"]
        ]
        ...
        pages[str(label)] = {
            ...
            "component_paths": {
                "historical": {
                    stage: str(path) for stage, path in historical_paths.items()
                },
                "public": {
                    stage: str(path) for stage, path in public_paths.items()
                },
            },
            ...
            "row_cluster_members": {
                "public": _member_rows(
                    public_members, public_components, historical_components
                ),
        ...
        json.dumps(
            {
                ...
                "pages": {
                    label: {
                        ...
                        "first_persisted_divergence": page[
                            "first_persisted_divergence"
                        ],
                    }
