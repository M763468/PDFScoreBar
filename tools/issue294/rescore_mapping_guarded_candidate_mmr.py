...
Issue #264 already established and persisted the accepted spatial rebase from the
historical MMR fixtures onto accepted physical measure geometry.  The original
version of this tool attempted to repeat that first rebase from the historical
``numbering_base.json`` tree, but those local Issue #120 artifacts are not a durable
dependency.

This tool therefore consumes the accepted Issue #264 geometry-rebased report as the
evaluation anchor, then spatially maps its accepted physical MMR bboxes onto the
Issue #294 mapping-guarded candidate geometry.  Retained Issue #294 actual MMR
overrides are independently rebased from their retained numbering geometry onto the
...
def _rebase_accepted_expected_to_candidate(
    ...
    *,
    ...
    for anchor in anchors:
        ...
        accepted_page_index = int(accepted_key_raw[0])
        if accepted_page_index != global_page_index:
            raise ValueError(
                f"Accepted mapping page mismatch: expected={global_page_index} "
                f"actual={accepted_page_index}"
            )
...
def run(
    ...
    for mode in MODES:
        ...
        bc_exact_by_mode[mode] = all(
            l["page_id"] == r["page_id"]
            and l["actual_rebased"] == r["actual_rebased"]
    ...
    payload = {
        "schema_version": "issue294.mapping_guarded_candidate_mmr_rescore.v2",
        "status": "completed",
        "execution_contract": {
