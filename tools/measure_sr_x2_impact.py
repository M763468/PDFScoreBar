...
def main():
    ...
    docker_prefix = [
        ...
        "sr_eval_gpu", "/opt/venv_sr/bin/python"
    ]
    ...
    cmd_x4 = docker_prefix + [
        ...
        "--images", rel_image, "--output-root", rel_output_x4,
        "--force-run-id", "sr4", "--enable-sr", "--sr-scale", "4",
        "--ground-truth", f"page_001:{rel_gt}"
    ]
    ...
    cmd_x2 = docker_prefix + [
        "src/homr_eval_scripts/homr_evaluator.py",
        "--images", rel_image, "--output-root", rel_output_x2,
        "--force-run-id", "sr2", "--enable-sr", "--sr-scale", "2",
        "--ground-truth", f"page_001:{rel_gt}"
    ]
    ...
    print("-" * 50)
    print(f"{'Duration (s)':<15} | {results['sr_x4']['duration']:>10.2f} | {results['sr_x2']['duration']:>10.2f} | {results['sr_x2']['duration']-results['sr_x4']['duration']:.2f}")
    print(f"{'Peak VRAM (MiB)':<15} | {results['sr_x4']['peak_vram']:>10} | {results['sr_x2']['peak_vram']:>10} | {results['sr_x2']['peak_vram']-results['sr_x4']['peak_vram']}")
    print(f"{'F1 Score':<15} | {results['sr_x4']['f1']:>10.4f} | {results['sr_x2']['f1']:>10.4f} | {results['sr_x2']['f1']-results['sr_x4']['f1']:.4f}")
    print("="*50)
