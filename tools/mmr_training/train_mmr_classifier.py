...
class TextNoiseOverlay:
    ...
    def __call__(self, image, staff_mask=None):
        ...
        for _ in range(self.max_attempts):
            ...
            draw.text((x, y), text, fill=(0, 0, 0), font=font, stroke_width=stroke_width, stroke_fill=(0, 0, 0))
            return img
...
class MMRDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.staff_mask_root:
            mask_path = self.staff_mask_root / f"{Path(img_path).stem}{self.staff_mask_suffix}{self.staff_mask_ext}"
            if mask_path.exists():
...
def train_model(args):
    ...
    terms = [
        "pizz.", "arco", "div.", "unis.", "solo", "tutti", "a 2",
        "cresc.", "dim.", "espress.", "dolce", "sempre", "sim.",
        "f", "p", "mf", "mp", "ff", "pp", "sfz",
        "Allegro", "Andante", "Largo", "Presto", "Moderato",
        "Tempo I", "Cadenza", "G.P.", "V.S.", "attacca",
        "rit.", "rall.", "accel.", "a tempo", "cant.", "marc.",
        "legg.", "stacc.", "ten.", "con sord.", "senza sord.",
        "sul G", "sul D"
    ]
    ...
    for epoch in range(args.epochs):
        ...
        print(f"Epoch {epoch+1}/{args.epochs}: Loss={epoch_loss:.4f} | Val Acc={val_acc:.4f} F1={val_f1:.4f} Prec={val_prec:.4f} Rec={val_rec:.4f} LR={lr:.6f}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--text-font", type=str, default=None, help="Path to a music text font (e.g. Bravura/Finale).")
    parser.add_argument("--text-font-dir", type=str, default=None, help="Directory with .ttf/.otf fonts for random sampling.")
    parser.add_argument("--text-fonts-zip", type=str, default=None, help="Zip file containing .ttf/.otf fonts for random sampling.")
