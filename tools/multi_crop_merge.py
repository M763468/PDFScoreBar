...
def main() -> None:
    ...
    ap.add_argument("--crop-list", type=Path, required=True, help="JSON list with crop detections and offsets")
    ...
    crop_entries = json.loads(args.crop_list.read_text())
    for entry in crop_entries:
        det_path = Path(entry["detections"])
        x_off = int(entry.get("x_offset", 0))
        y_off = int(entry.get("y_offset", 0))
        if det_path.exists():
            merged.extend(offset_boxes(load_homr_boxes(det_path), x_off, y_off))
