...
def organize_errors(eval_root, overrides_root, model_path, device):
    ...
    for page_dir in processed_pages:
        ...
        for g_idx, gt_count in gt_set.items():
            ...
            # OCR Analysis
            o_crop_raw = img[
                max(0, y1 - 80) : min(img.shape[0], y2 + 20),
                max(0, x1 - 20) : min(img.shape[1], x2 + 20),
            ]
            ...
            if key in pred_map:
                ...
                if prob < 0.5:
                    ...
                    cv2.putText(
                        o_vis,
                        f"OCR:{ocr_raw_txt} (No number found)",
                        (10, 60),
                        cv2.FONT_HERSHEY_SIMPLEX,
                        0.5,
                        (255, 0, 0),
        ...
        for key, pred_count in pred_map.items():
            if key not in matched_preds:
                ...
                cv2.putText(
                    c_vis,
                    f"Prob:{prob_fp:.3f} Hallucination",
                    (10, 30),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    0.7,
                    (0, 0, 255),
    ...
    with open(stage1_dir / "SUMMARY.md", "w") as f:
        ...
        for it in s1_items:
            f.write(
                f"| {it['Page']} | {it['Loc']} | {it['Type']} | {it['Prob']} | ![]({it['Img']}) |\n"
            )
    ...
    with open(stage2_dir / "SUMMARY.md", "w") as f:
        ...
        for it in s2_items:
            f.write(
                f"| {it['Page']} | {it['Loc']} | {it['Type']} | {it['GT']} | {it['Pred']} | {it['OCR']} | ![]({it['Img']}) |\n"
            )
