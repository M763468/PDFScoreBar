...
def main():
    det_file = Path(
        "logs/hybrid_generalization/repro_shostakovich_100/20260324_153852/hybrid_results/page_001_hybrid.json"
    )
    gt_file = Path(
        "data/evaluation2/annotations/Shostakovich-Festival_Overture_Va/page_001/boxes_sorted.json"
    )
    ...
    res = greedy_barline_match(
        preds, gts, rule_name="center_anchor", vov_threshold=0.5, xdist_threshold=15.0
    )
    print(
        f"TP: {len(res.matches)}, FP: {len(res.false_positive_indices)}, FN: {len(res.false_negative_indices)}"
    )
