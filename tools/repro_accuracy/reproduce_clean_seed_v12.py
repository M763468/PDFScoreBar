...
def main():
    ...
    for rec in records:
        ...
        baseline_json = hybrid_sub_root / "baseline" / "batch" / page_stem / f"{page_stem}_detections.json"
        ...
        if not omr_json.exists():
            omr_json = hybrid_sub_root / "omr_sr" / "batch" / page_stem / f"{page_stem}_detections.json"
        ...
        for boxes_list in [baseline_boxes, sr_boxes, omr_boxes]:
            ...
            for b in boxes_list:
                sb = (int(b[0]*dyn_scale), int(b[1]*dyn_scale), int(b[2]*dyn_scale), int(b[3]*dyn_scale))
        ...
        for b in sorted_boxes:
            if not any(barline_iou(b, fb) > 0.5 for fb in final_seeds):
