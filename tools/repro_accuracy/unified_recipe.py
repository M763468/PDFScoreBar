...
def main():
    ...
    baseline_json = (
        hybrid_run_root / "baseline" / "batch" / page_stem / f"{page_stem}_detections.json"
    )
    ...
    gt_file = (
        PROJECT_ROOT / "data/evaluation2/annotations" / score_name / page_stem / "boxes_sorted.json"
    )
    gts = []
    for item in json.load(open(gt_file)):
        if isinstance(item, list):
            gts.append(tuple(item[:4]))
        elif "barline_location" in item:
            gts.append(tuple(item["barline_location"]))

    res = greedy_barline_match(
        preds, gts, rule_name="center_anchor", vov_threshold=0.5, xdist_threshold=12.0
    )
    print(
        f"VERIFIED Result: TP: {len(res.matches)} | FP: {len(res.false_positive_indices)} | FN: {len(res.false_negative_indices)}"
    )
