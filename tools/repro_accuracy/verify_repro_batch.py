...
def get_gt_boxes(gt_data):
    boxes = []
    for item in gt_data:
        if isinstance(item, list): boxes.append(tuple(item[:4]))
        elif "barline_location" in item: boxes.append(tuple(item["barline_location"]))
    return boxes
...
def main():
    ...
    output_root = Path("logs/verify_repro_batch/probe_scan")
    ...
    for score in scores:
        ...
        # 1. Run Probe Scan (1x) using v12 seeds as bands_from
        ...
        for img in images:
            ...
            scored_file = output_root / f"eval2_{score}_{img.stem}" / "pipeline2_no_peak_scored.json"
            ...
            # Using 30.0 for xdist_threshold as in original
