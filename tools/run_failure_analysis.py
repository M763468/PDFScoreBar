...
def run_debug(work, page, measure, output_dir):
    ...
    # Paths
    json_path = Path(
        f"logs/experiments/batch_verification_20260107_v4/{work}/{page_str}/numbering_final.json"
    )
    ...
    if work == "prokofiev1":
        ...
        mask_root = Path(
            f"logs/hybrid_generalization/eval2_prokofiev1_{page_str}/baseline/{page_str}/{page_str}"
        )
    else:
        img_root = Path("data/evaluation2/images/prokofiev5")
        mask_root = Path(
            f"logs/hybrid_generalization/eval2_prokofiev5_{page_str}/baseline/{page_str}/{page_str}"
        )
...
def main():
    ...
    parser.add_argument(
        "--output-dir", type=Path, default=Path("logs/experiments/failure_analysis_20260107")
    )
