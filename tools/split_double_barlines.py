    ...
    for record in records:
        ...
        if bar_type == "double_barline" and bbox:
            ...
            if len(peaks) == 2:
                ...
                # Keep original
                print(f"Skipping double_barline at {bbox} in {Path(json_path).name}: Found {len(peaks)} peaks")
                final_records.append(record)
        else:
            final_records.append(record)
...
def main():
    ...
    parser.add_argument("--json-root", required=True, help="Root of annotations (e.g. data/evaluation2/annotations)")
    parser.add_argument("--image-root", required=True, help="Root of images (e.g. data/evaluation2/images)")
    parser.add_argument("--output-vis", default="logs/double_barline_vis", help="Directory to save visualizations")
    parser.add_argument("--apply", action="store_true", help="Apply changes to JSON files (default: dry-run)")
