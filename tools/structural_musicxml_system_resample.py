...
def count_musicxml_measures(path: Path) -> int:
    tree = ET.parse(path)
    root = tree.getroot()
    # choose first part
    part = root.find(".//part")
...
def main() -> None:
    ...
    total_measures = count_musicxml_measures(args.musicxml)
    num_systems = max(1, len(systems))
    base = total_measures // num_systems if num_systems else 0
    rem = total_measures % num_systems if num_systems else 0

    per_band_preds: Dict[int, List[int]] = {i: [] for i in range(len(bands))}
    system_metrics = []
    for sys_idx, sys_bands in enumerate(systems):
        target = base + (1 if sys_idx < rem else 0)
        union_xs: List[int] = []
