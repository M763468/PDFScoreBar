...
def main() -> None:
    ...
    for sys_idx, sys_bands in enumerate(systems):
        counts = [(i, len(per_band_xs.get(i, []))) for i in sys_bands]
        if counts:
            anchor_idx, anchor_count = max(counts, key=lambda t: t[1])
            anchor_xs = per_band_xs.get(anchor_idx, [])
        else:
            anchor_idx, anchor_count, anchor_xs = -1, 0, []
        for band_i in sys_bands:
            per_band_preds[band_i] = list(anchor_xs)
        system_metrics.append(
            {
                "system_index": sys_idx,
                "band_indices": sys_bands,
                "anchor_band": anchor_idx,
