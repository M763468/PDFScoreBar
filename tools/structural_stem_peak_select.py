...
def select_top_peaks(signal: np.ndarray, target: int, min_gap: int) -> List[int]:
    if target <= 0:
        return []
    indices = np.argsort(signal)[::-1]
    chosen: List[int] = []
    for idx in indices:
        if signal[idx] <= 0:
            break
...
def main() -> None:
    ...
    ap.add_argument("--stem-mask", type=Path, required=True)
    ...
    stem_mask = cv2.imread(str(args.stem_mask), cv2.IMREAD_GRAYSCALE)
    if stem_mask is None:
        raise SystemExit(f"Failed to load stem mask: {args.stem_mask}")
    ...
    for sys_idx, sys_bands in enumerate(systems):
        if not sys_bands:
            continue
        target = max((len(per_band_xs.get(i, [])) for i in sys_bands), default=0)
        ...
        if col_sum.size == 0 or target <= 0:
            ...
            chosen = select_top_peaks(col_sum, target, args.min_gap)
            chosen = [x_min + c for c in chosen]
        for band_i in sys_bands:
            per_band_preds[band_i] = list(chosen)
