...
print(f"SR items covered by Baseline: {sr_covered_by_base} / {len(sr_boxes)} ({sr_covered_by_base/len(sr_boxes):.2%})")

# Baseline vs OMR (Hypothetical No-SR Pipeline)
print("-" * 20)
base_supported_by_omr = get_intersection_count(base_boxes, omr_boxes, iou_thresh=0.1) # Loose threshold for line matching
