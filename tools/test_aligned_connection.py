...
def main():
    ...
    for i in range(len(staves) - 1):
            ...
            for b2 in s2.barlines:
                ...
                if abs(c1 - c2) <= ALIGN_TOL:
                    ...
                    if cv2.countNonZero(closed) > 0:
                        ...
                        cv2.line(
                            debug_vis,
                            (int((x_start + x_end) / 2), y1_bot),
                            (int((x_start + x_end) / 2), y2_top),
                            (0, 255, 0),
                            2,
                        )
                    else:
        ...
        cv2.putText(
            debug_vis, label, (50, (y1_bot + y2_top) // 2), cv2.FONT_HERSHEY_SIMPLEX, 0.8, color, 2
        )
