...
def process_page(image_path: Path, mask_path: Path, output_path: Path):
    ...
    print(f"Processed {image_path.name} -> {len(systems)} systems (Divisi: {[len(s.staves) for s in systems if len(s.staves)>1]})")

def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--image-dirs", nargs='+', required=True, help="Directories containing images (e.g., data/evaluation2/images/prokofiev1)")
    ...
    for img_path in images:
        ...
        if pred_matches:
            ...
            print(f"Warning: No hybrid_predictions.json found for {page_name}. Connectivity check will fail.")
