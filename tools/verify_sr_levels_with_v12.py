...
def run_test(tag, image_path, sr_enabled, sr_scale):
    ...
    if sr_enabled:
        if sr_scale == 4:
            ...
            sr_img = (
                PROJECT_ROOT
                / "logs/hybrid_generalization/sr_x2_test/eval2_sr_x2_test_split/sr/batch/page_001/page_001.png"
            )
    ...
    if sr_enabled:
        ...
        current_images_root = (
            PROJECT_ROOT
            / "logs/full_pipeline_runs/bypass_sr_test/eval2_bypass_sr_test_v2/inputs/images"
        )
    ...
    run_scoring_batch(
        ...
        bands_from=None
        if sr_enabled
        else bands_from,  # Bypass geo-filter for SR due to coords mismatch
    ...
    print(
        f"RESULT {tag}: P={len(match_result.matches) / (len(match_result.matches) + len(match_result.false_positive_indices)):.4f}, R={len(match_result.matches) / len(gt_boxes):.4f}, TP={len(match_result.matches)}, FN={len(match_result.false_negative_indices)}"
    )
