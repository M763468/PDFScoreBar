...
def extract_staff_bands(mask_path: Path, target_size: Tuple[int, int], min_height: int = 10) -> List[Tuple[int, int, int, int]]:
    """
...
def run_visualization():
    ...
    output_path = Path("logs/gt_rebuild_hybrid_eval/20260102T134300_best_repro_fullparams/per_page/page_10/measure_numbering_overlay_final.png")
    ...
    for sys in page.systems:
        ...
        for m in sys.measures:
            if m.start_bar: all_processed_barlines.add(m.start_bar)
            if m.end_bar: all_processed_barlines.add(m.end_bar)
    ...
    for bar in all_processed_barlines:
        ...
        cv2.rectangle(overlay, 
                      (bar.bbox.x1, bar.bbox.y1), 
                      (bar.bbox.x2, bar.bbox.y2), 
                      color, thickness)
    ...
    for system in page.systems:
        ...
        for measure in system.measures:
            ...
            # Draw number
            cv2.putText(overlay, text, (draw_x, draw_y), 
                        cv2.FONT_HERSHEY_SIMPLEX, font_scale, (0, 150, 0), thickness, cv2.LINE_AA)
            
            # Draw faint measure boundary for debug
            cv2.line(overlay, (measure.bbox.x1, text_y_base), (measure.bbox.x1, text_y_base+5), (0, 255, 0), 1)
            cv2.line(overlay, (measure.bbox.x2, text_y_base), (measure.bbox.x2, text_y_base+5), (0, 255, 0), 1)
