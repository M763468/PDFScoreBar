...
def main():
    parser = argparse.ArgumentParser(description="Visualize extracted ROIs for multi-measure rests with notehead context.")
    ...
    parser.add_argument("--output-image", type=Path, required=True, help="Path to save overlay image")
    parser.add_argument("--threshold", type=int, default=50, help="Max pixels of notehead to consider 'empty'")
    parser.add_argument("--vertical-margin", type=int, default=80, help="Vertical margin (px) to check above/below staff")
    parser.add_argument("--erode-iter", type=int, default=1, help="Iterations of erosion to remove noise")
        ...
        for system in page["systems"]:
            for measure in system["measures"]:
                ...
                if pixel_count <= args.threshold:
                    ...
                    cv2.putText(overlay, f"REST M{measure['number']}", (x1, roi_y1 - 10), 
